...
static const double FALLBACK_ROTATE_3D = 2.0 / 3.0;
static const double ROTATE_3D_ORTHOGONALIZER = UNSKEW_2D;

static const float SKEW_4D = -0.138196601125011f;
static const float UNSKEW_4D = 0.309016994374947f;
static const float LATTICE_STEP_4D = 0.2f;

static const double NORMALIZER_3D = 0.07969837668935331;
static const double NORMALIZER_2D = 0.01001634121365712;
static const double NORMALIZER_4D = 0.0220065933241897;

static const float RSQUARED_2D = 0.5f;
static const float RSQUARED_3D = 0.6f;
static const float RSQUARED_4D = 0.6f;
...
int OpenSimplex2::fastFloor(double x) {
    int xi = (int)x;
    return x < xi ? xi - 1 : xi;
}

int OpenSimplex2::fastRound(double x) {
...
void OpenSimplex2::initializeGradients2d()
{
    static float grad2d[] = {
    ...
    for (int i = 0; i < ARRAY_LENGTH(grad2d); i++) {
        grad2d[i] = (float)(grad2d[i] / NORMALIZER_2D);
    }
    for (int i = 0, j = 0; i < ARRAY_LENGTH(GRADIENTS_2D); i++, j++) {
        if (j == ARRAY_LENGTH(grad2d)) j = 0;
        GRADIENTS_2D[i] = grad2d[j];
    }
}


void OpenSimplex2::initializeGradients3d()
{
    static float grad3d[] = {
    ...
    for (int i = 0; i < ARRAY_LENGTH(grad3d); i++) {
        grad3d[i] = (float)(grad3d[i] / NORMALIZER_3D);
    }
    for (int i = 0, j = 0; i < ARRAY_LENGTH(GRADIENTS_3D); i++, j++) {
        if (j == ARRAY_LENGTH(grad3d)) j = 0;
        GRADIENTS_3D[i] = grad3d[j];
    }
}

void OpenSimplex2::initializeGradients4d()
{
    static float grad4d[] = {
    ...
    for (int i = 0; i < ARRAY_LENGTH(grad4d); i++) {
        grad4d[i] = (float)(grad4d[i] / NORMALIZER_4D);
    }
    for (int i = 0, j = 0; i < ARRAY_LENGTH(GRADIENTS_4D); i++, j++) {
        if (j == ARRAY_LENGTH(grad4d)) j = 0;
        GRADIENTS_4D[i] = grad4d[j];
    }
}
